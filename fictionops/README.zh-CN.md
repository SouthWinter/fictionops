...
14. 读 [0.1.0 发布说明](docs/release-notes-0.1.0.zh-CN.md)，了解发布边界和已知限制。
15. 读 [兼容性策略](docs/compatibility.zh-CN.md)，理解哪些 CLI 和 JSON 行为可以被脚本或 controller 依赖。
16. 读 [已知限制](docs/known-limits.zh-CN.md)，理解哪些事情 FictionOps 当前不保证。
17. 读 [PyPI 发布说明](docs/pypi-release.zh-CN.md)，了解 trusted publishing、凭据隔离和回滚记录。
18. 读 [发布演练证据模板](docs/release-trial-evidence.zh-CN.md)，了解如何记录 GitHub Actions run、artifact、TestPyPI URL、安装烟测和最终结论；publish workflow 也会生成独立的 release trial evidence draft artifact，避免证据草稿混入 wheel/sdist 发布包。
19. 读 [持续 dogfood 周期证据模板](docs/dogfood-cycle-evidence.zh-CN.md)，了解 1.0 前如何记录收口后的真实项目维护周期。
20. 读 [稳定窗口证据模板](docs/stability-window-evidence.zh-CN.md)，了解 1.0 前如何记录兼容性窗口。
21. 读 [1.0 剩余执行清单](docs/stable-core-remaining-checklist.zh-CN.md)，明确哪些工作已经只是维护，哪些必须等真实外部发布、dogfood 和稳定窗口证据。
22. 然后复制 `templates/` 里的模板，建立自己的长篇项目，或读 [长篇大纲迁移案例](examples/long_novel_outline_migration_case_zh.md)，看一部复杂长篇如何从混乱材料变成可维护结构。
