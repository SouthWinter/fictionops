...
- `agent continue` also consumes persistent counterevidence provenance. Grounded open issues select `prepare_counterevidence_revision`; evidence-blocked issues select `retrieve_counterevidence`; model-withdrawn-only projects select author-owned `review_model_withdrawals`. Existing candidate, budget, failure, cancellation, canon-sync, and stale-memory precedence remains intact. `--execute` does not execute these R1-R3 actions.
