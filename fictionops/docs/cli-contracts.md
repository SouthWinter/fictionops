...
- `agent counterevidence reverify` runs one independent model decision per evidence-ready deduplicated request, returning `uphold`, `withdraw`, or `still_insufficient`. It enforces a pre-call budget, allows at most one bounded schema repair per malformed result within that budget, records runner receipts and tokens, and deterministically downgrades resolved verdicts whose exact quotations do not occur in supplied evidence. It never edits manuscript or grants model output author authority.
