...
- `agent continue` also consumes persistent counterevidence provenance. Grounded open issues select preparation, staged output advances to independent verification, failed verification returns to bounded revision, and a passing candidate stops at author-owned review/acceptance. Evidence-blocked issues select retrieval; model-withdrawn-only projects select author-owned withdrawal review. Existing candidate, budget, failure, cancellation, canon-sync, and stale-memory precedence remains intact. `--execute` does not execute these R1-R3 actions.
...
- `agent benchmark` runs repeated `raw`, `rag`, `full`, `no_memory`, `no_guard`, and `no_contract` review conditions through one runner without exposing fixture answers. Benchmark v2 reports positive and negative controls through precision, recall, accuracy, false-positive rate, and grounded evidence. `--blind-out` and `--blind-key-out` write an anonymous human-review packet and its condition mapping separately. `agent failure-lab` injects bounded state, artifact, budget, receipt, reviewer, and story-contract failures in temporary workspaces and reports detection, recovery, and protected-hash outcomes.
...
- `agent-revise-workflow` packages a chapter, review-workflow report, and revision contract for a model-backed revision runner; after comprehensive review, an independent preservation verifier classifies every finding as `uphold`, `withdraw`, or `needs_counterevidence`. Deterministic self-abstention checks override model claims such as `No change needed` inside an issue. Original findings, verifier decisions, and effective issues remain separate artifacts; only upheld issues reach the reviser. `--no-preservation-verify` disables the extra model call but keeps deterministic checks. The workflow then creates a persistent session, issue snapshots, diff, before/after audits, at most the configured targeted retries, and a model-backed semantic invariant check. `--max-model-calls` (default 12) and `--max-runtime-seconds` (default 1800) are hard controller budgets recorded in `model_budget.json`; optional `--max-total-tokens`, `--max-cost`, and `--cost-currency` stop before the next call once receipt-backed cumulative usage reaches the threshold. It never applies the revised text automatically.
- `agent guard set` creates or updates a stable author-authority guard under `.fictionops/author_guards.json`; `agent guard retire` preserves its history while removing its authority from future runs, and `agent guards` lists the registry. Only active `G-*` ids from this registry can authorize a model withdrawal. Reviewer-generated preserve constraints remain untrusted suggestions.
- `agent counterevidence export` turns `needs_counterevidence` findings into an anonymous annotation packet and stores fixture identity/control labels in a separate private key. It accepts either a revision run directory or preservation evidence plus benchmark/fixture files. `agent counterevidence score` rejects incomplete or mismatched annotations and reports decisions, grounding, repair-harm risk, human effort, and agreement or label challenges against inherited case-level controls; it does not misreport those controls as issue-level ground truth.
- `agent counterevidence escalate` selects human-`insufficient` findings (or all findings in an unannotated packet), collapses exact duplicate findings, routes each claim to full-chapter, adjacent-paragraph, knowledge-source, character-memory, or author-intent evidence, and retrieves bounded source material when a chapter/project is available. Missing sources remain explicit `needs_source`; the command never fabricates evidence or edit prose.
- `agent counterevidence reverify` runs one independent model decision per evidence-ready deduplicated request, returning `uphold`, `withdraw`, or `still_insufficient`. Before each call it compiles an assertion-scoped evidence window: exact-quote claims receive neighboring paragraphs, retrieved character/canon evidence remains scope-routed, and genuinely chapter-level claims receive one deduplicated full chapter. Active author guards remain visible. `--max-evidence-chars` defaults to 16000. Reports record strategy, before/after context characters, prompt size, runner receipts, and tokens. Grounding accepts quotations only from the compiled prompt-visible window. One bounded schema repair is allowed within the hard model-call budget. The command never edits manuscript or grants model output author authority.
- `agent counterevidence benchmark-windows FIXTURES --runner ...` performs paired `full_context` and `window` re-verification with the same model-facing decision contract. Hidden expected verdicts and control notes are never rendered into prompts. It reports expected-verdict accuracy, grounded resolution, evidence recall, verdict agreement, and input/evidence reductions overall and by scope. Benchmark output is evidence only and never mutates manuscript or issue state.
- `agent counterevidence apply` hash-checks the source packet, escalation artifact, and unchanged revision source; recomputes grounding from the packet plus retrieved evidence; then writes effective verdicts to the persistent issue ledger: `uphold -> open`, `withdraw -> model_withdrawn`, and `still_insufficient -> evidence_blocked`. Only open issues enter the generated reviser queue. Author-owned accepted/rejected/waived states and existing addressed/verified resolutions are preserved. Application is dry-runnable, idempotent per run, auditable, and never edits prose.
- `agent counterevidence prepare-revision RUN_DIR` validates the application/queue relationship, unchanged chapter hash, and current ledger state before creating a standard `agent-exec` bundle. The bundle contains only grounded `open` upholds, exact repair evidence, active author guards, and the source chapter; it excludes withdrawn, blocked, resolved, and author-protected issues and does not rerun comprehensive review or edit the manuscript.
- `agent counterevidence verify-revision BUNDLE [--runner ...]` first checks title, bounded scope, and newly introduced local repetition deterministically. A failing preflight writes `model_call_count=0` and needs no runner. Only a passing preflight sends the issue contract plus complete unified diff, not duplicate full source/candidate chapters, to an independent runner; exact candidate-grounded quotations, no unrelated changes, preserved active author guards, no new canon, and unchanged hashes remain mandatory. It writes a verification artifact but never edits prose or the ledger.
- `agent counterevidence accept-revision BUNDLE` is the explicit author boundary. It rechecks bundle/contract/source/candidate hashes and current grounded-open ledger states, supports a non-writing `--dry-run`, atomically applies only a verified candidate, and records `addressed -> verified -> accepted` issue transitions.
- `agent counterevidence repair-revision BUNDLE --runner ...` is available only after failed verification reports a local prose regression. The runner returns exact unique `old_quote -> new_quote` replacements instead of a full chapter; FictionOps checks unique anchoring, forbidden sequences, replacement size, remaining local regressions, and the bounded-change envelope before updating only the staged candidate. Failed whole-candidate retries are archived, and byte-identical retries stop as no progress.
