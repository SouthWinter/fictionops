...
- `agent counterevidence verify-revision BUNDLE --runner ...` asks an independent runner to judge only the contracted fixes, then requires exact candidate-grounded quotations, no reported unrelated changes, preserved active author guards, no new canon, an unchanged source, and a deterministic bounded-change envelope. It writes a verification artifact but never edits prose or the ledger.
