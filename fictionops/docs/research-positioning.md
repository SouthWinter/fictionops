...
FictionOps is a long-horizon creative workflow harness. It uses fiction as the domain, but the research problem is broader: how can an AI-assisted or agentic system operate over a large, evolving human project without losing context, corrupting source-of-truth files, or hiding decisions in a chat transcript?
...
FictionOps is not a model and not an autonomous novelist. It is an operating layer for persistent creative workspaces:
...
The research contribution is the boundary design: a model can participate in a long project without being allowed to silently become the project memory, editor, publisher, and authority at the same time.
...
- model comparisons are not yet complete;
...
FictionOps is a local-first workflow harness for long-horizon writing agents. It turns a large fiction project into a persistent workspace with scoped context packs, explicit task envelopes, staged model outputs, audit gates, and publish artifacts. The key research question is how agents can assist complex creative projects while preserving human authority, recoverability, and long-term state consistency.
...
- It can be connected to external runners, APIs, local models, or controllers.
