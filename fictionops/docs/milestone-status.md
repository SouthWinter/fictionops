...
| --- | --- | --- | --- |
| 0.1.0 Pre-Alpha MVP | Complete locally | `docs/completion-audit-0.1.0.zh-CN.md`, `docs/release-notes-0.1.0.zh-CN.md`, 133 tests, wheel/sdist builds, source and built-wheel install smoke tests. | External release record if publishing is chosen. |
...
| 0.7.0 Writing Agent Commands | Partially proven | AI-first `write-chapter`, `revise-chapter`, and `audit-chapter` commands now wrap `agent-run`, optional `agent-exec`, and `agent-inbox` with staged output tests. | Session persistence, richer command telemetry, real provider dogfood, and `agent-session` style multi-step orchestration. |
