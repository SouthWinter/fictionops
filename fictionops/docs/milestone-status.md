...
| --- | --- | --- | --- |
| 0.1.0 Pre-Alpha MVP | Complete locally | `docs/completion-audit-0.1.0.zh-CN.md`, `docs/release-notes-0.1.0.zh-CN.md`, 158 tests in the current regression suite, wheel/sdist builds, source and built-wheel install smoke tests. | External release record if publishing is chosen. |
...
| 0.6.0 AI Provider Onboarding | Complete locally | OpenAI-compatible Chat runner v1, major provider presets, `setup-ai`, key-free env examples, AI-first README quickstart, real DeepSeek API runs, and secret-isolation tests/scans. | Native provider usage/token adapters remain enhancements, not 0.6 blockers. |
| 0.7.0 Writing Agent Commands | Complete locally | Unified agent commands, closed write/revision loops, persistent sessions, attributed trajectories, explicit controller policy, causal simulation, independent review, budgets, selective rewrites, hash-guarded acceptance, and checkpoint-aware resume/cancel. | Retry-internal resume and batch scheduling remain optional P4 enhancements. |
| 0.8.0 Agent Runtime Dogfood | Partially proven | Real DeepSeek reports plus a reproducible raw/RAG/full/ablation harness, unified trajectory schema, seven-scenario failure lab, and interview-ready failure case. | Repeated real-model baseline results, AI-assisted publish preparation, author review time, and final accepted edit distance. |
