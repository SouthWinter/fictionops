...
The current suite covers 50 CLI commands and 126 regression tests. It checks:
...
- agent prompt, connector kit generation, connector smoke tests, run, exec, inbox, next-step controller, agent workflow preflight audit, no-model controller loop workflows, and OpenAI Responses runner dry-run wiring;
