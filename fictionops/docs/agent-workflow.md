...
This document answers a narrow but important question: once FictionOps is connected to AI, does FictionOps itself count as an agent workflow?

Short answer: **yes, when it is connected to a model runner or controller, the whole system becomes an agentic workflow; FictionOps core remains the workflow harness, not the autonomous agent.**
...
An external AI runner or controller does the agent work:

- reads a task bundle;
- calls a model or local agent;
...
| `agent-run` + external model runner + `agent-inbox` | bounded agent workflow | A model performs a scoped task, and FictionOps captures the result for review. |
...
| Controller loops through multiple steps but keeps staged outputs and gates | autonomous-adjacent workflow | Automation can advance work, but authority remains outside model output. |
...
- **Context scope:** the agent receives only the files needed for the task.
- **Role boundary:** the prompt says whether the agent is drafting, auditing, planning, or publishing.
...
That means FictionOps is not competing to be a "better novelist agent." It is the operating layer that lets different agents participate without silently breaking a long project.

## Practical Answer

If you connect an AI model only as a helper for one command, call the setup **AI-assisted FictionOps**.

If you connect an external runner that receives task bundles and returns staged outputs, call it a **FictionOps agent workflow**.

If you connect a controller that reads `agent-next` JSON and chains multiple commands, call it a **FictionOps agentic workflow**. For a normal writing project, `agent-next` chooses from migration, inbox, chapter, revision, and publishing steps. For the FictionOps package itself, it chooses from stable-core evidence action items and stops on external release, dogfood, or stability-window work that cannot be fabricated locally.
