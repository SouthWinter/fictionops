...
- AI workflow evidence: `eval-agent` was run on `ch_002`, and the agent harness stopped at the human-review boundary with staged output remaining quarantined. On 2026-07-10, a real OpenAI-compatible DeepSeek runner checkpoint was added in the private sandbox for `book_01/ch_002` using `audit-chapter --role info-boundary-auditor`; the model output was staged in an agent run directory, `agent-inbox` reported `ready_for_review`, and no manuscript or canon file was overwritten. The public evidence does not claim autonomous direct-writing.
