...
| 里程碑 | 状态 | 当前证据 | 剩余证据 |
| --- | --- | --- | --- |
| 0.1.0 Pre-Alpha MVP | 本地已完成 | `docs/completion-audit-0.1.0.zh-CN.md`、`docs/release-notes-0.1.0.zh-CN.md`、139 个测试、wheel/sdist 构建、源码安装和 built-wheel 安装烟测。 | 如果决定发布，还需要外部发布记录。 |
...
| 0.6.0 AI Provider Onboarding | 部分证明 | OpenAI-compatible Chat runner v1、provider preset、模型供应商文档、dry-run 路径、`setup-ai`、无密钥 env 示例，以及证明 preset/env-file 接线和不写入真实 key 的测试。 | AI-first README quickstart，以及默认用户故事中的真实调用路径。 |
| 0.7.0 Writing Agent Commands | 部分证明 | AI-first `write-chapter`、`revise-chapter`、`audit-chapter` 和 `agent-session` 已覆盖暂存任务创建、可选 runner 执行、inbox 检查和持久 session 台账，并有测试。 | 更完整 telemetry 和真实供应商 dogfood。 |
