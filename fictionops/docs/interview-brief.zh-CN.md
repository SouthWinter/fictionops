...
> 最新的 Agent 科研实习讲法、三分钟/十分钟脚本和失败案例见 [`interview-agent-script.zh-CN.md`](interview-agent-script.zh-CN.md) 与 [`interview-agent-research-case.zh-CN.md`](interview-agent-research-case.zh-CN.md)。本文保留较完整的历史项目证据与通用问答。

这份文档用于在 agent 研究或应用 AI 工程面试中介绍 FictionOps。它刻意短、证据导向，方便快速组织回答。

## 60 秒介绍

FictionOps 是一个面向长篇小说项目的有状态 Agent。它不是一键小说生成器，而是由确定性 controller、外部项目记忆、模型 API runner、因果/场景规划、独立审读、验证预算和人工采纳门禁组成。它解决的核心问题是长程项目维护：一部长篇会不断积累大纲、正史、人物记忆、信息边界、修订历史、模型交接和发布产物。
...
- 它是 Agent 系统，不是重新训练的写作模型；workflow harness 是它可解释、可复现的工程底座。
...
算。当前 `fictionops agent write|revise|accept|continue` 已把 controller、API runner、项目记忆、工具调用、验证、预算和停止条件连成持久闭环。更准确的边界是：它是有状态 Agent，但不是拥有正文/正史最终权力的自主小说家。
