...
| Package release evidence exists outside the local checkout | Local wheel/sdist builds, CI/publish workflows, accepted `docs/release-trial-evidence.md`, GitHub Actions run `28849146871`, TestPyPI `fictionops==0.1.1`, clean venv install smoke, and `audit-release-evidence` reporting `ready=true`. | Complete with external evidence | Keep future release trials recorded with real run URLs, package hashes, install smoke, reviewer, and decision. |
