...
The test suite currently covers 62 CLI commands and 160 regression tests, including the unified agent entry, persistent issue identity and author decisions, high-risk semantic fixtures, positive/negative benchmark controls, blind-review packet isolation, preservation-aware issue verification, phase/hash checkpoints, auditable cancellation, typed memory, causal simulation, story-fact ledgers, state-aware scene drafting, selective rewriting, hard execution budgets, grounded adversarial review, closed-loop revision/writing, and hash-guarded acceptance.
